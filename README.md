...
- **방어 코드 미구현**: 공격 시나리오만 실습했고, 실제 Node.js나 Spring Boot 코드에서 어떻게 방어하는지 직접 작성해보지 못했습니다. 공격 원리를 아는 것과 방어 코드를 직접 구현하는 것은 다른 역량입니다.
- **SQL Injection 실습 미완**: HTML Injection·XSS 위주로 실습했고, 웹 취약점의 대표격인 SQL Injection은 다루지 못했습니다.
- **환경 의존성**: VirtualBox + Kali + bee-box 구성이 복잡해 환경 재현이 어렵습니다. Docker로 구성했다면 훨씬 간편했을 것입니다.

## 고민해야 할 점

- **입력 검증의 위치**: 클라이언트(프론트엔드)에서 입력을 검증하는 것과 서버에서 검증하는 것의 차이가 뭘까요? 클라이언트 검증은 사용자 편의를 위한 것이고, 보안상 반드시 서버 측 검증이 필요합니다. 실습에서 Medium 레벨 우회를 보면서, 클라이언트 필터링만으로는 충분하지 않다는 것을 직접 확인했습니다.
- **CSP 헤더의 적용 범위**: Content-Security-Policy 헤더로 어디서 불러오는 스크립트만 실행할지 지정할 수 있는데, 이를 설정하는 구체적인 방법과 실제 서비스에서 어떻게 적용하는지 더 공부하고 싶습니다.
- **보안과 편의성의 트레이드오프**: `httpOnly` 쿠키는 JS에서 접근 불가해 XSS를 막지만, 일부 프론트엔드 기능(ex. 토큰 직접 접근)이 제한됩니다. 보안을 강화할수록 개발 편의성이 떨어지는 경우가 많은데, 어떤 기준으로 균형을 잡아야 하는지 고민이 필요합니다.
